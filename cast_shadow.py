...
### --------------------------------------------------------       
    def addPixmap(self, img):  ## the scene is cleared each new image file                     
        ...
        img = self.addPilShadow(name, img.width(), img.height())
        self.addCv2Shadow(img)
     
    ### --------------------------------------------------------    
    def addPilShadow(self, name, w, h):  ## replace color with gray
    ...
    def addCv2Shadow(self, img):  ## initial shadow - no perspective
        img = np.array(img)        
        img = cv2.cvtColor(img, cv2.COLOR_RGBA2BGRA)  ## convert pil to cv
 ...
 ### --------------------------------------------------------  
    def updateShadow(self): 
        img = self.getPerspective()
        self.addUpdatedShadow(img)   
        
    def getPerspective(self):       
        self.cpy = np.array(self.cpy)  
        self.cpy = cv2.cvtColor(self.cpy, cv2.COLOR_RGBA2BGRA)  ## convert pil copy to cv
        ...
        M = cv2.getPerspectiveTransform(src,dst) ## give it room to move around   
